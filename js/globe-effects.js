/* Fragment assembly: cinematic electronic entry, soft tactile sliding. */
(() => {
    'use strict';
    window.SiliqGlobeEffects = function(scene, surface, motion) {
        ...
        function energySweep(at,duration,pan,detune){
            ...
            oscillator.frequency.exponentialRampToValueAtTime(440,at+duration*.72);
            oscillator.frequency.exponentialRampToValueAtTime(880,at+duration);
            filter.type='lowpass';filter.Q.value=.8;filter.frequency.setValueAtTime(360,at);
            filter.frequency.exponentialRampToValueAtTime(2800,at+duration*.84);
            filter.frequency.exponentialRampToValueAtTime(1300,at+duration);
            gain.gain.setValueAtTime(0,at);gain.gain.linearRampToValueAtTime(.065,at+duration*.18);
            gain.gain.linearRampToValueAtTime(.13,at+duration*.76);gain.gain.exponentialRampToValueAtTime(.0001,at+duration);
            route(oscillator,gain,pan,filter);oscillator.start(at);oscillator.stop(at+duration+.04);
        }
        function energyResolve(at){
            const oscillator=context.createOscillator(),gain=context.createGain();oscillator.type='sine';
            oscillator.frequency.setValueAtTime(146.83,at);oscillator.frequency.exponentialRampToValueAtTime(55,at+.28);
            gain.gain.setValueAtTime(0,at);gain.gain.linearRampToValueAtTime(.24,at+.018);
            gain.gain.exponentialRampToValueAtTime(.0001,at+.7);
            route(oscillator,gain);oscillator.start(at);oscillator.stop(at+.74);
        }
        function assemblySound(){
            ...
            const build=Math.max(.12,remaining-.24);
            // Stereo charging sweep, accelerating digital notes, then the sphere locks into place.
            energySweep(t,build,-.55,-7);energySweep(t,build,.55,7);
            air(t,build,1.1,0,true);
            [329.63,493.88,659.25,739.99,987.77,1318.51].forEach((f,i)=>{
                const position=1-Math.pow(1-(i+1)/7,1.5);
                tone(f,t+position*build*.82,.23,.065,(i%2===0?-.45:.45));
            });
            const settle=t+build;
            energyResolve(settle);
            [659.25,987.77,1318.51,1975.53].forEach((f,i)=>tone(f,settle+i*.055,.85-i*.09,.085-i*.013,-.45+i*.3));
        }
        function cue(){
